...
fn runServer(server: *server_t) void {
    ...
    const rx_bytes_wifi_file = std.fs.openFileAbsolute("/sys/class/net/wlan0/statistics/rx_bytes", .{ .mode = .read_only }) catch unreachable;
    var reader = rx_bytes_wifi_file.reader();
    bytes_len = reader.readAll(&buf) catch unreachable;
    rx_bytes = std.fmt.parseUnsigned(server_t.stored_t, buf[0 .. bytes_len - 1], 10) catch unreachable;
    rx_bytes_wifi_file.close();
    const tx_bytes_wifi_file = std.fs.openFileAbsolute("/sys/class/net/wlan0/statistics/tx_bytes", .{ .mode = .read_only }) catch unreachable;
    reader = rx_bytes_wifi_file.reader();
    bytes_len = reader.readAll(&buf) catch unreachable;
    tx_bytes = std.fmt.parseUnsigned(server_t.stored_t, buf[0 .. bytes_len - 1], 10) catch unreachable;
    tx_bytes_wifi_file.close();
    const rx_bytes_ether_file = std.fs.openFileAbsolute("/sys/class/net/wlan0/statistics/rx_bytes", .{ .mode = .read_only }) catch unreachable;
    reader = rx_bytes_ether_file.reader();
    bytes_len = reader.readAll(&buf) catch unreachable;
    rx_bytes += std.fmt.parseUnsigned(server_t.stored_t, buf[0 .. bytes_len - 1], 10) catch unreachable;
    rx_bytes_ether_file.close();
    const tx_bytes_ether_file = std.fs.openFileAbsolute("/sys/class/net/wlan0/statistics/tx_bytes", .{ .mode = .read_only }) catch unreachable;
    reader = rx_bytes_ether_file.reader();
    bytes_len = reader.readAll(&buf) catch unreachable;
    tx_bytes += std.fmt.parseUnsigned(server_t.stored_t, buf[0 .. bytes_len - 1], 10) catch unreachable;
    ...
    std.debug.print("{} up ({}%), {} down ({}%)\n", .{ tx_bits, tx_percent, rx_bits, rx_percent });
